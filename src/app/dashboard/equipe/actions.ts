...
export async function inviteMemberAction(
  ...
  try {
    ...
    // For security, we create the user with a temporary random password.
    // The user will need to reset it. In a real scenario, we'd send an invitation link.
    ...
    console.log(`Usuário criado: ${email} com senha temporária: ${tempPassword}. É recomendável instruir o usuário a resetar a senha no primeiro login.`);

    return { success: true, data: { userId: userRecord.uid } }
  } catch (error) {
    console.error("Erro ao convidar membro:", error)
    return { success: false, error: "Falha ao convidar membro. Tente novamente." }
  }
}
